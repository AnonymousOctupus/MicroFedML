...
# (Modified from dict_keyexchange)
def keyexchange(peer_id, self_id, my_pkey, my_skey, peer_pkey):
    if peer_id > self_id:
      common_key_raw, _ = nb.crypto_kx_client_session_keys(my_pkey, my_skey, peer_pkey)
    else:
      _, common_key_raw = nb.crypto_kx_server_session_keys(my_pkey, my_skey, peer_pkey)

    # Hash the common key.
    comkey = int.from_bytes(nb.crypto_hash_sha256(common_key_raw), byteorder='big')
