...
from agent.secagg_group.SA_ServiceAgent import SA_ServiceAgent as ServiceAgent
...
class SA_ClientAgent(Agent):
  ...
  def __init__(self, id, name, type,
    ...
    # Finite field for RO and discrete log
    # if prime == -1:
  ...
  def receiveMessage (self, currentTime, msg):
    ...
    # Receiving the mutual mask key exchange information from all peers
        ...
        for peer_id in mk_shares_cipher_l:
            ...
            # h = self.finite_group.add(h, self.peer_mk[peer_id])
        ...
        for peer_id in self.group_member_list[self.group_id]:
            ...
            # h_share_ciphers[peer_id] = []
            # for rho in range(self.seg_num):
            #     byte_share = h_seg_shares[rho][peer_id].to_bytes(256, "big")
            #     h_share_ciphers[peer_id].append(
            #             self.peer_secret_box[peer_id].encrypt(byte_share))
    ...
    # Sending the offline set with the signature to members of neighboring groups
        ...
        for peer_id in r_ciphers:
            ...
            self.peer_h_share[peer_id] = int.from_bytes(byte_share, "big")
  ...
  def aggMaskedInput(self):
    ...
    self.current_base = self.ro.query(self.current_iteration, 1)[0]
