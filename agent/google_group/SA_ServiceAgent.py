...
import util.graph_generation as gg
...
class SA_ServiceAgent(Agent):
  ...
  def __init__(self, id, name, type,
    ...
    self.global_threshold = int(num_clients * 2 / 3) + 1
    if num_neighbors == -1:
      self.num_neighbors = self.num_clients
    else:
      self.num_neighbors = num_neighbors
    self.neighbor_threshold = int(num_neighbors * 2 / 3) + 1
    # The threshold for the server is different from that for the users
    if global_threshold != -1:
      self.global_threshold = global_threshold
    if neighbor_threshold != -1:
      self.neighbor_threshold = neighbor_threshold
    self.graph = gg.generate_semi_honest_graph(self.num_neighbors, self.num_clients)
  ...
  def receiveMessage (self, currentTime, msg):
    ...
    # Round 2: Receiving encrypted shares from a user i
      ...
      for id in self.user_pubkeys:
        if id in self.graph.adj[sender_id]:
  ...
  def keyExchange (self, currentTime):
    ...
    if len(self.user_pubkeys) < self.global_threshold:
      print(f"ERROR-Server: Not enough public keys." \
      f"(iteration {self.current_iteration}, num of pubkeys: {len(self.user_pubkeys)})")
      self.current_iteration = -1
    else:
      # Accumulate into offline setup.
      ...
      for id in self.user_pubkeys:
        # Each online user should receive its neighbors' pubkeys
        neighbors_pubkeys = {k:v for k, v in self.user_pubkeys.items()
                                if k in set(self.graph.adj[id])
                                or k == id}
        # print(f"Server: {len(neighbors_pubkeys)} pubkeys are sent to {id}")
        self.sendMessage(id,
                Message({"msg" : "PUBKEYS",
                        "pubkeys" : neighbors_pubkeys,
                        }),
                tag = "comm_key_generation_server")
        self.recordBandwidth(neighbors_pubkeys, 'PUBKEY')
      ...
      # print("finished sending public keys")
  ...
  def shareCipher (self, currentTime):
    ...
    if len(self.user_shares_cipher) < self.global_threshold:
      print(f"ERROR-Server: Not enough users have ciphers for them." \
      f"(iteration {self.current_iteration}, num of users receiving ciphers: {len(self.user_shares_cipher)})")
      self.current_iteration = -1
      return
    ...
    # print("num of shares for each user", len(self.user_shares_cipher[1]))
    self.current_round = 3
    self.setWakeup(currentTime + self.round_time + cal_time)
    # print("finished sending share ciphers")
    # print(pd.Timestamp('now'))


  def maskedInput (self, currentTime):
    dt_protocol_start = pd.Timestamp('now')
    # print("wake up in masked input")
    # print(pd.Timestamp('now'))
    ...
    if len(self.user_masked_input) < self.global_threshold:
    ...
    self.online_set = set(random.sample(self.online_set, self.num_online))
    ...
    for id in self.online_set:
        online_i = self.online_set.intersection(
                                set(self.graph.adj[id]))
        online_i.add(id)
        self.sendMessage(id,
        ...
        self.recordBandwidth(online_i, 'ONLINE_SET')
  ...
  def sharesReconstruct (self, currentTime):
    dt_protocol_start = pd.Timestamp('now')
    print(f"server receives shares for {len(self.user_shares)} clients")

    # Check if enough number of shares are received
    # If not, skip the iteration
    if len(self.user_shares) < self.global_threshold:
    ...
    for id in self.user_pubkeys:
      if id in self.online_set:
        ...
        h_mask = 0
        for peer_id in {k:v for k, v in self.user_pubkeys.items()
                                if k in set(self.graph.adj[id])
                                or k == id}:
...
# ======================== UTIL ========================
  ...
  def generateGraph(self, k, n):
      graph = nx.generator.harary_graph.hkn_harary_graph(k, n)
      print(graph)
