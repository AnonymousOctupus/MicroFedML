...
from agent.secagg_ro.SA_ClientAgent import SA_ClientAgent as ClientAgent
...
# Sydney max distance
# sydney_latency = np.random.uniform(low = 21000, high = 53000000, size=(len(agent_types),len(agent_types)))
...
model_args = { 'connected'   : True,
               ...
               # Headquarter in NYC, clients all over the worlds.
...
if not exists(file_name):
  with open(file_name, 'a') as results_file:
    ...
    title += "Client Bdw Offline,Client Bdw Online R1,Client Bdw Online R2,"
...
with open(file_name, 'a') as results_file:
  ...
  line += f"{(results['bdw_pubkey'] + results['bdw_shares']) / num_clients},"
  line += f"{results['bdw_masked_input'] / (num_clients * num_iterations)},"
  line += f"{results['bdw_r_share_sum'] / (num_clients * num_iterations)},"
  line += f"{results['srv_bdw_pubkey'] + results['srv_bdw_shares']},"
