...
from agent.google_group.SA_ServiceAgent import SA_ServiceAgent as ServiceAgent
...
class SA_ClientAgent(Agent):
  ...
  def __init__(self, id, name, type,
    ...
    if num_neighbors == -1:
      self.num_neighbors = self.num_peers
    else:
      self.num_neighbors = num_neighbors
    if threshold == -1:
      self.threshold = int(num_neighbors * 2 / 3) + 1
    else:
  ...
  def receiveMessage (self, currentTime, msg):
    ...
    # Receiving encrypted shares from the other clients
        ...
        if len(ciphers) < self.threshold:
            ...
            print(f"ERROR-Client {self.id}: Not enough masked input are received. ({len(cipher)})")
            self.current_iteration = -1
            return
    ...
    elif msg.body['msg'] == "ONLINE_SET" and self.current_iteration != 0:
        dt_protocol_start = pd.Timestamp('now')
    ...
    elif msg.body['msg'] == "OUTPUT" and self.current_iteration != 0:
        dt_protocol_start = pd.Timestamp('now')
  ...
  def maskedInput(self):
    ...
    # print("user", self.id, "'s r-mask':", prg.prg(seed = self.self_r)[0])
