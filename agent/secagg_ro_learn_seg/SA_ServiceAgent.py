...
class SA_ServiceAgent(Agent):
  ...
  def receiveMessage (self, currentTime, msg):
    ...
    # Round 2: Receiving the sums of the shares from the user i
        ...
        for i in range(len(msg.body['shares_sum'])):
          self.user_shares_sum[i][sender_id] = msg.body['shares_sum'][i]
  ...
  def aggShareSum (self, currentTime):
    ...
    if len(self.user_shares_sum[0]) < self.threshold:
      self.current_iteration = -1
      return

    r_sum = [0]*self.weight_length
    for i in range(self.weight_length):
      r_sum[i] = self.ss.reconstructInExponent(self.user_shares_sum[i], self.threshold)
    # r_sum = self.ss.reconstructInExponent(self.user_shares_sum, self.threshold)

    masked_sum = [None] * self.weight_length
    for i in range(self.weight_length):
      masked_sum[i] = [None, None]
      masked_sum[i][0] = [1] * self.segNumInt
      masked_sum[i][1] = [1] * self.segNumDec
      
    for id in self.online_set:
      for i in range(self.weight_length):
        for j in range(self.segNumInt):
          masked_sum[i][0][j] = self.finite_field_p.multiply(self.user_masked_input[id][i][0][j], masked_sum[i][0][j])
        for j in range(self.segNumDec):
          masked_sum[i][1][j] = self.finite_field_p.multiply(self.user_masked_input[id][i][1][j], masked_sum[i][1][j])


    base = self.ro.query(self.current_iteration, self.weight_length)

    x_sum = [None] * self.weight_length
    for i in range(self.weight_length):
      x_sum[i] = [None, None]
      x_sum[i][0] = [None] * self.segNumInt
      x_sum[i][1] = [None] * self.segNumDec
      for j in range(self.segNumInt):
        x_sum[i][0][j] = self.finite_field_p.log_brute_force_with_neg(self.finite_field_p.divide(masked_sum[i][0][j], r_sum[i]), base[i])
      for j in range(self.segNumDec):
        x_sum[i][1][j] = self.finite_field_p.log_brute_force_with_neg(self.finite_field_p.divide(masked_sum[i][1][j], r_sum[i]), base[i])
        # x_sum[i] = self.finite_field_p.log_brute_force_with_neg(self.finite_field_p.divide(masked_sum[i], r_sum[i]), base[i])
        # x_sum[i] = float(x_sum[i]) / self.num_clients
