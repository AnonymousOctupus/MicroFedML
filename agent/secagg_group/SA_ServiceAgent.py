  ...
  def aggShareSum (self, currentTime):
    ...
    for d in range(self.num_groups):
      ...
      mask_sum_d = self.ss.reconstructInExponent(self.user_shares_sum[d],
                              self.threshold)
      mask_sum = self.finite_field_p.multiply(mask_sum, mask_sum_d)


    base = self.ro.query(self.current_iteration, 1)[0]
    masked_sum = 1
    for id in self.online_set:
      masked_sum = self.finite_field_p.multiply(self.user_masked_input[id], masked_sum)
    x_sum = self.finite_field_p.log(
                    self.finite_field_p.divide(masked_sum, mask_sum),
                    base)

    # result = segmenting.combine(x_sum, self.seg_length)
    result = x_sum
    result = self.finite_group.add(result, self.h_mask)
